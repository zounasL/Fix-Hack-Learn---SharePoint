...
public class AuthenticationActivity extends WebViewClient {
    ...
    private Boolean getMsToken = false;
    ...
    public boolean shouldOverrideUrlLoading(WebView view, String url) {
        ...
        if(url.contains("code=") && url.startsWith(redirectUri) && !this.getMsToken) {
            String code = this.codeCollector(url);
            this.getMsToken = true;
            this.getToken(code, false, view);
        } else if(url.contains("code=") && url.startsWith(this.activity.getString(R.string.msRedirectUri))) {
            String code = this.codeCollector(url);
            this.getToken(code, true, view);
        }
        return false;
    }
    ...
    public void getToken(final String code, final Boolean isMsAuth, WebView view) {
        ...
        try {
            ...
            if(isMsAuth) {
                url = this.activity.getString(R.string.msMicrosoftonline);
                clientId = this.activity.getString(R.string.msClientId);
                redirectUri = this.activity.getString(R.string.msRedirectUri);
            }
            ...
            StringRequest stringRequest = new StringRequest
                    (Request.Method.POST, url + "/token", new Response.Listener<String>() {
                        ...
                        public void onResponse(String response) {
                            if(!isMsAuth) {
                                tokens = response;
                                view.loadUrl(activity.getString(R.string.microsoftonline) + "/authorize?client_id=" + activity.getString(R.string.msClientId) + "&response_type=code&redirect_uri=" + activity.getString(R.string.msRedirectUri) + "&response_mode=query&scope=" + activity.getString(R.string.tenant) + "/.default%20openid%20offline_access");
                            } else {
                                closeAuth(response);
                            }
                        }
